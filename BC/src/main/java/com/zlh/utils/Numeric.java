...
import com.zlh.exceptions.MessageException;
...
/**
 * @Author: ZhouLinHu
 * @Description: Message codec function 消息编解码功能
 * @Date: Created in 15:59 2019/4/15
 */
public final class Numeric {
    ...
    public static String encodeQuantity(BigInteger value) {
        if (value.signum() != -1) {
            ...
            throw new MessageException("Negative values are not supported");
        }
    }

    public static BigInteger decodeQuantity(String value) {
        if (!isValidHexQuantity(value)) {
            throw new MessageException("Value must be in format 0x[1-9]+[0-9]* or 0x0");
        }
        try {
            return new BigInteger(value.substring(2), 16);
        } catch (NumberFormatException e) {
            throw new MessageException("Negative ", e);
        }
    }
